...
class Recipe:
    def __init__(self, title, ingredients, cooking_time, dietary_info, instructions):
        self.title = title
        self.ingredients = ingredients
        self.cooking_time = cooking_time
        self.dietary_info = dietary_info
...
class RecipeManager:
    ...
    def save_data(self):
        with open("recipes.json", "w") as file:
            json.dump([recipe.__dict__ for recipe in self.datas], file)

    def load_data(self):
        try:
            with open("recipes.json", "r") as file:
                recipe_data = json.load(file)
                self.datas = [Recipe(**data) for data in recipe_data]
        except FileNotFoundError:
            self.datas = []

    def add_recipe(self, recipe):
        self.datas.append(recipe)
        self.update_recipe()
        self.save_data()
    ...
    def update_recipe(self):
        choice_listbox.delete(0, tk.END)
        for recipe in self.datas:
            choice_listbox.insert(tk.END, recipe.title)

    def view_recipe(self, index):
        recipe = self.datas[index]
        title.set(recipe.title)
        ingredients.set(recipe.ingredients)
        cookingtime.set(recipe.cooking_time)
        dietaryinfo.set(recipe.dietary_info)
        instructions_entry.delete(1.0, tk.END)
        instructions_entry.insert(1.0, recipe.instructions)


def add_recipes():
    recipe = Recipe(title.get(), ingredients.get(), cookingtime.get(), dietaryinfo.get(), instructions_entry.get(1.0, "end-1c"))
    manager.add_recipe(recipe)

def view_recipes():
    index = int(choice_listbox.curselection()[0])
    manager.view_recipe(index)

def delete_recipes():
    index = int(choice_listbox.curselection()[0])
    manager.delete_recipe(index)

def reset():
    index = int(choice_listbox.curselection()[0])
    recipe = Recipe(title.get(), ingredients.get(), cookingtime.get(), dietaryinfo.get(), instructions_entry.get(1.0, "end-1c"))
    manager.datas[index] = recipe
...
recipes_frame = tk.LabelFrame(frame, text="Recipes")
recipes_frame.grid(row=0, column=0, padx=20, pady=20, sticky="news")

title_label = tk.Label(recipes_frame, text="Title")
title_label.grid(row=0, column=0)

cookingtime_label = tk.Label(recipes_frame, text="Cooking Time")
cookingtime_label.grid(row=1, column=0)

dietaryinfo_label = tk.Label(recipes_frame, text="Dietary information")
dietaryinfo_label.grid(row=2, column=0)

ingredients_label = tk.Label(recipes_frame, text="Ingredients")
ingredients_label.grid(row=3, column=0)

instructions_label = tk.Label(recipes_frame, text="Instructions")
instructions_label.grid(row=4, column=0)

title = tk.StringVar()
cookingtime = tk.StringVar()
ingredients = tk.StringVar()
dietaryinfo = tk.StringVar()
instructions = tk.StringVar()

title_entry = tk.Entry(recipes_frame, textvariable=title)
cookingtime_entry = tk.Entry(recipes_frame, textvariable=cookingtime)
dietaryinfo_entry = tk.Entry(recipes_frame, textvariable=dietaryinfo)
ingredients_entry = tk.Entry(recipes_frame, textvariable=ingredients)
instructions_entry = tk.Text(recipes_frame, width=20, height=10)
...
listofrecipes_frame = tk.LabelFrame(frame, text="List of Recipes")
listofrecipes_frame.grid(row=0, column=2, padx=20, pady=20, sticky="news")

choice_label = tk.Label(listofrecipes_frame, text="Choose your recipe")

choice_listbox = tk.Listbox(listofrecipes_frame, height=20)
scrollbar = tk.Scrollbar(listofrecipes_frame)
...
buttons_frame = tk.LabelFrame(frame, text="Recipe Editor")
buttons_frame.grid(row=0, column=4, padx=20, pady=20, sticky="news")

viewbutton = tk.Button(buttons_frame, text="View a recipe", font="arial 12 bold", command=view_recipes)
addbutton = tk.Button(buttons_frame, text="Add a recipe", font="arial 12 bold", command=add_recipes)
deletebutton = tk.Button(buttons_frame, text="Delete a recipe", font="arial 12 bold", command=delete_recipes)
editbutton = tk.Button(buttons_frame, text="Edit a recipe", font="arial 12 bold", command=reset)
